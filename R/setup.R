...
#' @param p Numeric. 0 < p < 1. Lifetime percentile to be estimated at the 
#'  use condition, i.e. stress levels are 0.
...
#' @param reparam Logical. Whether reparameterization is applied to model 
...
#'  \code{design_type = "locally"}. Default is TRUE.
#' 
...
#' @param early_stopping Integer. The frequency, i.e. number of iterations, 
#'  of validating the design optimality using equivalence theorem. 
#'  The optimization process stops once maximum directional derivative is 
#'  approximately 1.
#' @param tol Numeric. Convergence tolerance. The algorithm stops if 
#'  \code{abs(max_directional_derivative - 1) < tol}.
...
#' @param w_var Numeric. A number between \eqn{[0, 1]} that controls the 
#'  percentage of iterations during which PSO linearly decrease inertia
#'   weight from \code{w0} to \code{w1}. Default value is 0.8.
...
#'
#' Sets initial particles for PSO, initial locally optimal design, and 
#' initial parameters for Nelder-Mead algorithm.
#'
#' @param init_swarm Optional matrix of initial particle positions. If not 
#'  defined, particle positions are randomly generated using \code{runif} 
#'  with pre-determined number of particles \code{n_swarm} and particle size.
#' @param init_local Optional vector of initial locally optimal design for 
#'  Nelder-Mead optimization. If not defined, the initial vector representing 
#'  locally optimal design is \code{c(1, 0.6, 0.3)}. 
#' @param init_coef_mat Optional matrix of initial parameters to implement 
...
#'  \code{10 * as.matrix(expand.grid(rep(list(c(1, -1)), j_factor + 1)))}.
#'  
